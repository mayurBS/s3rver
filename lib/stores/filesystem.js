...
class FilesystemStore {
  ...
  getResourcePath(bucket, key = '', resource) {
    ...
    const suffix = format(parts.pop(), resource).trim();
  ...
  async putMetadata(bucket, key, metadata, md5) {
    const metadataPath = this.getResourcePath(bucket, key, 'metadata.json');
    const md5Path = this.getResourcePath(bucket, key, '.md5');
  ...
  async listObjects(bucket, options) {
    ...
    const objectSuffix = format('', '').trim();
    const keys = [];
    let isTruncated = false;
    for (const keyPath of it) {
      // console.log("🚀 :===== keyPath:", keyPath)
      if (keyPath.endsWith(" .md5") || keyPath.endsWith(" metadata.json")) {
        continue;
      }
    ...
    if (replacementMetadata) {
      ...
      if (srcObjectPath !== destObjectPath) {
        await Promise.all([
          ...
          fs.copy(
            this.getResourcePath(srcBucket, srcKey, '.md5'),
            this.getResourcePath(destBucket, destKey, '.md5'),
          ),
        ]);
      }
      return this.getMetadata(destBucket, destKey);
  ...
  async deleteObject(bucket, key) {
    await Promise.all(
      [
        ...
        this.getResourcePath(bucket, key, '.md5'),
