...
class FilesystemStore {
  ...
  async getObject(bucket, key, options) {
    try {
      ...
      const catalogMetadata = {
        ...
        'x-amz-meta-inCache': catalog.inCache ?? "",
        'x-amz-meta-onTarget': catalog.onTarget ?? "",
  ...
  async putObject(object) {
    ...
    try {
      const catalog = await axios.post("http://localhost:3000/s3/catalogs", JSON.stringify(catalogData), { headers: apiHeaders})
